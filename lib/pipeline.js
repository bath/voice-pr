...
import { MODES, normalizeMode } from "./mode.js";
 ...
 */
export async function runBatch({ prRef, transcript, mode = MODES.AUTHOR }, emit = () => {}) {
  ...
  if (mode === MODES.REVIEWER) return runReviewerBatch(pr, transcript, emit);
...
/**
 * Shared orchestrator submit: file the work item, nudge the mayor, track it to
 * a merge, then post the intent trail host-side (pogod reaps the polecat at
 * merge, so post-merge work can't run in the polecat itself).
 */
 ...
 * routes author mode through the orchestrator; reviewer mode posts comments only.
 * @param {{prRef:string, segments:Array<{text,file,line}>, mode?:string}} input
 */
export async function runSession({ prRef, segments, mode = MODES.AUTHOR }, emit = () => {}) {
  ...
  if (mode === MODES.REVIEWER) return runReviewerSession(pr, segments, emit);
...
export async function runReviewerBatch(
  pr,
  transcript,
  emit = () => {},
  ...
  const body =
    `🎙️ **Voice review comments**\n\n${transcript.trim()}\n\n` +
    `_Reviewer mode: posted as PR comments only; no commits were made._`;
  emit("review-commenting", { count: 1 });
  const comment = await github.postIssueComment(pr, body);
  const result = reviewOnlyResult(pr, {
    summary: "Posted 1 PR comment in reviewer mode. No commits were made.",
    reviewComments: [
      {
...
export async function runReviewerSession(
  ...
  segments,
  emit = () => {},
  github = { postAnchoredComment, postIssueComment }
) {
  ...
  for (const segment of segments) {
    ...
    if (!text) continue;
    const line = Number(segment.line);
    const hasLine =
      segment.line !== null &&
      segment.line !== undefined &&
      String(segment.line).trim() !== "" &&
      Number.isFinite(line);
    if (segment.file && hasLine && pr.headRefOid) {
      emit("review-commenting", { file: segment.file, line });
      const comment = await github.postAnchoredComment(pr, {
        body: reviewOnlyBody(text),
  ...
  if (loose.length) {
    emit("review-commenting", { count: loose.length });
    const body =
      `🎙️ **Voice review comments without diff anchors**\n\n` +
      loose.map(formatLooseReviewComment).join("\n") +
      `\n\n_Reviewer mode: posted as PR comments only; no commits were made._`;
    const comment = await github.postIssueComment(pr, body);
  ...
  const result = reviewOnlyResult(pr, {
    summary: `Posted ${count} review comment${count === 1 ? "" : "s"} in reviewer mode. No commits were made.`,
    reviewComments,
  });
  emit("done", result);
  return result;
...
export function reviewOnlyBody(text) {
  return `🎙️ **Voice review comment**\n\n${text}\n\n_Reviewer mode: no commits were made._`;
}

function reviewOnlyResult(pr, { summary, reviewComments }) {
  return {
    backend: "reviewer",
    mode: MODES.REVIEWER,
    status: "done",
...
function formatLooseReviewComment(segment) {
  const text = (segment.text || "").trim();
  const loc = segment.file
    ? `**\`${segment.file}${segment.line ? `:${segment.line}` : ""}\`** — `
    : "";
  return `- ${loc}${text}`;
}
