...
async function submitToAgentUnlocked(
  ...
  if (
    ...
    scheduleTrailComment({ pr, commits: outcome.commits, emit });
  ...
  const result = {
    ...
    summary: outcome.commits?.length
      ? outcome.backend === "menial-fast-path"
        ? `Applied anchored edit on ${pr.headRefName} without agent inference.`
...
function scheduleTrailComment({ pr, commits, emit }) {
  const lines = commits
