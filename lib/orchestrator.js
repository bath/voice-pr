...
// then nudge the mayor so it dispatches on its current cycle instead of on its
// next passive pass, and track the item to a merge.
...
/**
 * Fail fast with a clear, actionable error if the mayor's Claude token is
 * expired (or about to expire). Without this gate a dispatch with a dead token
 * still files a work item and signals the mayor, which 401s and never dispatches
 * — so the run silently stalls for the whole DISPATCH_TIMEOUT_MS and ends in an
 * ambiguous "timeout" (issue #10). Reusing the same expiry read the preflight
 * uses, we surface it up front instead. A missing/unreadable token file can't be
 * assessed and does not block (see mayorTokenState).
 */
export async function assertMayorAuth() {
  const tok = await mayorTokenState();
  if (!tok.ok) throw new Error(tok.detail);
}
...
/**
 * Signal the mayor that a work item is ready to dispatch — by PTY nudge, so it
 * runs a coordination cycle *now* rather than noticing the item on its next
 * passive pass. Mail was too slow: a mailed ask sits until the mayor's next
 * cycle, and voice-pr items are latency-sensitive (the author is watching the PR
 * and expects commits in minutes, not on the mayor's own clock). `pogo nudge
 * <name> <message>` writes the ask into the mayor's PTY (waiting for it to go
 * idle first) and — per `pogo nudge` — automatically falls back to mail if the
 * mayor isn't running, so we keep the attributed-ask guarantee for free.
 */
export async function signalMayor({ id, pr, headRef, reason }) {
  ...
  const message = reason
    ...
    : `voice-pr filed ${id} (${detail}, assignee=mayor). Please dispatch a polecat for it now.`;
  await dx(["pogo", "nudge", "mayor", message], { allowFail: true });
}

/**
 * Track the work item to a terminal state, emitting progress. Resolves with
 * { status, refinery, commits, via } where status is done/failed/timeout and
 * `via` records which signal ended the track (commits | status | refinery).
 *
 * Completion is NOT solely the mg/refinery poll — commits landing on the PR
 * branch is the ground truth the user watches, so `opts.commitsLanded` (an async
 * probe returning the commits that landed since dispatch started) is a
 * first-class terminal signal. A real merge always advances the UI even when the
 * mayor goes idle without flipping `mg show`/refinery to a terminal state.
 * @param {{commitsLanded?: () => Promise<Array>}} [opts]
 */
export async function trackWorkItem(id, repoPath, emit, opts = {}) {
  ...
  for (;;) {
    ...
    // Ground truth: did the polecat's commits actually land on the PR branch?
    // This wins over the poll status so a real merge advances promptly even if
    // `mg show`/refinery never flips terminal (stale item, mayor stopped, token
    // expired mid-run) — the "stuck on Orchestrator working for 12 min" bug.
    if (opts.commitsLanded) {
      const landed = await opts.commitsLanded();
      if (landed && landed.length) {
        emit("commits-landed", { id, count: landed.length });
        return { status: "done", item, refinery: ref, commits: landed, via: "commits" };
      }
    }

    if (status === "done" || ref?.status === "merged")
      return { status: "done", item, refinery: ref, via: status === "done" ? "status" : "refinery" };
    if (ref?.status === "failed")
      return { status: "failed", item, refinery: ref, via: "refinery" };
