...
// pipeline, the Cursor agent runtime — logs to that session's tracer via getTracer()
...
function consoleDetail(code, detail) {
  if (detail?.message) return ` ${detail.message}`;
  if (!/(?:cache|workspace-prepare)/.test(code) || !detail) return "";
  const short = (sha) => (sha ? String(sha).slice(0, 8) : "none");
  let summary =
    detail.reason ||
    detail.action ||
    (code.endsWith("-hit")
      ? `${detail.state || "ready"} cache entry found`
      : null);
  if (detail.fromHeadSha !== undefined || detail.toHeadSha !== undefined) {
    summary = `${summary ? `${summary}; ` : ""}${short(detail.fromHeadSha)} → ${short(detail.toHeadSha)}`;
  } else if (detail.headSha || detail.cachedHeadSha) {
    summary = `${summary ? `${summary}; ` : ""}head ${short(
      detail.headSha || detail.cachedHeadSha
    )}`;
  }
  if (Number.isFinite(detail.ageMs))
    summary = `${summary ? `${summary}; ` : ""}age ${detail.ageMs}ms`;
...
export class Tracer {
  ...
  event(code, detail = {}, level = "info") {
    ...
    // can fire repeatedly during workspace setup and transcription — high
    // volume, low signal — so those info lines are
    ...
    if (!(level === "info" && code.startsWith("exec.") && !process.env.VOICE_PR_TRACE_EXEC)) {
      ...
      const msg = consoleDetail(code, detail);
...
export const CODE_MAP = {
  "bridge.": "server.js — HTTP endpoints (prepare/warm/context/preflight/transcribe/dispatch)",
  "exec.": "lib/exec.js — child-process spawns (gh, git, ffmpeg, whisper)",
  "stt.": "lib/transcribe.js — local whisper transcription",
  "pipeline.": "lib/pipeline.js — session → PR resolution → warm-agent submit",
