...
import { applyMenialEdit, menialFastPathEnabled, planMenialEdit } from "./menial.js";
...
const DEFAULT_PIPELINE_VARIANT =
  process.env.VOICE_PR_PIPELINE_VARIANT || "single-turn";
...
export function createAgentRuntime(options = {}) {
  ...
  const pipelineVariant = configuredPipelineVariant(
    options.pipelineVariant ?? DEFAULT_PIPELINE_VARIANT
  );
  ...
  async function doWarm(entry, emit) {
    try {
      ...
      entry.recordToAgentReadyMs = entry.recordStartedAt
        ...
        : null;
      entry.recordToAgentWarmingMs =
        entry.pipelineVariant === "prewarm"
          ? entry.recordToAgentReadyMs
          : null;
      const agentReadyDetail = {
        ...
        preparationHit: entry.preparationHit,
        preparationAgeMs: entry.preparationAgeMs,
        recordToAgentWarmingMs: entry.recordToAgentWarmingMs,
        recordToAgentReadyMs: entry.recordToAgentReadyMs,
        pipelineVariant: entry.pipelineVariant,
      };
      if (entry.pipelineVariant === "single-turn") {
        entry.readyAt = Date.now();
        entry.state = "ready";
        emit("agent-staged", {
          ...agentReadyDetail,
      ...
      emit("agent-warming", agentReadyDetail);
  ...
  async function executeEntry(entry, { pr, context, segments, emit }) {
    ...
    try {
      ...
      const menialPlan = planMenialEdit(segments);
      if (menialPlan.eligible) {
        emit("menial-attempt", {
          file: menialPlan.segment.file,
          line: menialPlan.segment.line ?? null,
          oldValue: menialPlan.oldValue,
          newValue: menialPlan.newValue,
          lineScoped: menialPlan.lineScoped,
        });
        const menial = await applyMenialEdit({
          plan: menialPlan,
          workspace: entry.workspace,
        });
        if (menial.applied) {
          emit("menial-applied", {
            file: menial.file,
            oldValue: menial.oldValue,
            newValue: menial.newValue,
          });
          await runCommand("git", ["add", "--", menial.file], {
            cwd: entry.workspace,
          });
          await runCommand(
            "git",
            ["commit", "-m", menial.commitMessage],
            { cwd: entry.workspace }
          );
          ...
          entry.result = {
            ...
            commits,
            agentSummary:
              `MENIAL APPLIED\n${menial.file}: ${menial.oldValue} → ${menial.newValue}`,
            metrics: {
        ...
        emit("menial-fallback", { reason: menial.reason });
      } else if (menialFastPathEnabled()) {
        emit("menial-fallback", { reason: menialPlan.reason });
      }
      ...
      const executionRun = await entry.agent.send(
        buildExecutionPrompt({
          ...
          prewarmed: entry.inferenceWarmed,
        }),
        {
          mode: "agent",
      ...
      entry.result = {
        ...
        metrics: {
          ...
          staleHeadRefresh: sync.changed,
          pipelineVariant: entry.pipelineVariant,
          inferenceTurnsBeforeStop: entry.inferenceWarmed ? 1 : 0,
        },
      };
      emit("agent-finished", {
        commits: commits.length,
  ...
  async function check() {
    ...
    try {
      ...
      const value = {
        ok: true,
        detail: `authenticated · model ${modelLabel(model)} · ${models.length} available · variant ${pipelineVariant}`,
      };
  ...
  return { preparePr, warm, execute, check, status, shutdown, pipelineVariant };
}
...
function publicStatus(entry) {
  return {
    ...
    pipelineVariant: entry.pipelineVariant,
    inferenceWarmed: entry.inferenceWarmed,
...
function configuredPipelineVariant(value) {
  if (value === "prewarm" || value === "single-turn") return value;
  throw new Error(
    `VOICE_PR_PIPELINE_VARIANT must be "prewarm" or "single-turn", got "${value}"`
  );
}
