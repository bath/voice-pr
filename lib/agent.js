...
export function createAgentRuntime(options = {}) {
  ...
  async function doWarm(entry, emit) {
    try {
      ...
      const model = options.model || process.env.VOICE_PR_MODEL || "auto";
      ...
      emit("agent-warming", { agentId: entry.agentId, model });
    ...
    try {
      ...
      if (afterHead !== beforeHead) {
        ...
        if (remoteHead !== afterHead)
          throw new Error(
            `Cursor agent committed ${afterHead.slice(0, 8)} but did not push it to ${pr.headRefName}`
          );
      }
  ...
  async function check() {
    ...
    try {
      ...
      const value = {
        ok: true,
        detail: `authenticated · model ${model} · ${models.length} available`,
      };
