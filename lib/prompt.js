...
the request with those new commits.`
  ...
  const orientation = prewarmed
    ? `The recording ended. Interpret and execute the author's spoken review
comments against the PR context you already analyzed.`
    : `This is the only inference turn. The workspace and PR metadata were prepared
...
perform broad PR, repository, test-suite, Jira, or GitHub re-discovery.`;
  const contextReference = prewarmed
    ? "repository context you already loaded"
    : "provided PR metadata and the prepared workspace";
  const gatheringRule = prewarmed
    ? "Do not repeat context gathering you already completed."
    : "Do not broaden context gathering beyond what the anchored requests require.";
  return `${orientation}
...
PR diff, and the ${contextReference}. The anchor is evidence about
...
- Do not broaden scope or refactor adjacent code. ${gatheringRule}
...
Context source: ${contextReference}. Jira ${context.jiraKey || "not detected"}; CI ${context.checksSummary || "unknown"}.`;
}
