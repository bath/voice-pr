...
are already available to you, briefly consult the \`atlassian\` MCP server for the
Jira ticket and the \`slack\` MCP server for related PR/repo discussion to sharpen
intent — if they are not present, skip them entirely and do NOT spend time
searching. Never let context-gathering delay the edits.
${optional.join("\n")}`;
}
